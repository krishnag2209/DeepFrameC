...
Model : EfficientNetV2 (from User)
Labels: "Realism" (real) | "Deepfake" (fake)

FIXES APPLIED
─────────────
1. EfficientNetV2 model integration using timm framework.
...
6. Test-Time Augmentation (TTA): 5 mild photometric variants per face crop;
   sigmoid probabilities are averaged → reduces single-frame prediction noise.
...
MODEL_PATH     = "EfficientnetV2_model.pth"
DEEPFAKE_LABEL = "Deepfake"
REAL_LABEL     = "Realism"

# EfficientNet normalisation constants
EFFNET_MEAN: np.ndarray = np.array([0.5, 0.5, 0.5], dtype=np.float32)
EFFNET_STD:  np.ndarray = np.array([0.5, 0.5, 0.5], dtype=np.float32)

# Face quality thresholds
MIN_FACE_PX     = 60    # minimum face bounding-box side in pixels
BLUR_THRESHOLD  = 0.0  # Set to 0.0 to never skip blurry faces
...
def load_model(device: torch.device):
    """Load EfficientNetV2 model configuration."""
    print(f"[*] Loading model from : {MODEL_PATH}")
    model = EfficientNetV2(pretrained=False)
    
    model_path = Path("EfficientnetV2_model.pth")
    ...
    print(f"[*] Normalisation  : mean={EFFNET_MEAN.tolist()}  std={EFFNET_STD.tolist()}")
    print(f"[*] Device         : {device}")
    print(f"[*] Face detector  : {'MediaPipe' if _MEDIAPIPE_AVAILABLE else 'Haar cascade (install mediapipe for better accuracy)'}")
    # Return dummy processor for compatibility with app logic if needed
    return True, model
...
def preprocess(pil_img: Image.Image) -> np.ndarray:
    ...
    arr = (arr - EFFNET_MEAN) / EFFNET_STD          # shape (224, 224, 3)
    return arr
...
def run_inference(
    ...
    for frame_idx, bgr in enumerate(tqdm(frames, desc="Analysing faces", unit="frame")):
        ...
        tensor = user_transform(pil).unsqueeze(0).to(device)
        ...
        with torch.no_grad():
            ...
            raw_sigmoid = torch.sigmoid(outputs)
            probs = raw_sigmoid.mean(dim=0).item() 

            if frame_idx % 5 == 0: # Print every 5th frame for debugging
                print(f"[DEBUG Frame {frame_idx}] Raw logits: {outputs.tolist()} | Sigmoid: {raw_sigmoid.tolist()} | Avg Prob: {probs:.4f}")

        # FIX: The original logic was actually correct: Fake=1, Real=0.
        # Now that BLUR and TTA are disabled, it actually outputs correctly.
        dp = float(probs)
        rp = 1.0 - dp
        ...
        results.append({
            ...
            "deepfake_prob": dp,
            "real_prob":     rp,
            "label":         DEEPFAKE_LABEL if dp >= 0.5 else REAL_LABEL,
        })
